    ...
    >
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title">{{ isEdit ? 'Befund bearbeiten' : 'Befund hinzufügen' }}</h3>
        ...
        <form @submit.prevent="handleSubmit" class="modal-body">
          <div class="form-group">
            ...
            <textarea
              v-model="form.text"
              rows="4"
              placeholder="Befundbeschreibung"
              class="form-input"
              required
            />
          </div>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" :disabled="isSubmitting" class="btn-primary">
              {{ isSubmitting ? 'Speichern...' : isEdit ? 'Änderungen speichern' : 'Befund hinzufügen' }}
            </button>
          </div>
        </form>
      </div>
...
const createMutation = useMutation({
  mutationFn: (data: any) => patientApi.createFinding(props.patientId, data),
  onSuccess: () => {
  ...
  onError: (err) => {
    error.value = err instanceof Error ? err.message : 'Fehler beim Erstellen des Befunds';
  }
});

const updateMutation = useMutation({
  mutationFn: ({ findingId, data }: { findingId: string; data: any }) =>
    patientApi.updateFinding(props.patientId, findingId, data),
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ['patient-findings', props.patientId] });
    emit('success');
    handleClose();
  ...
  onError: (err) => {
    error.value = err instanceof Error ? err.message : 'Fehler beim Aktualisieren des Befunds';
  }
});
